  ...
  async createAppDeployment(args: {
    ...
    try {
      ...
      if (err instanceof UniqueIntegrityConstraintViolationError) {
        // TODO: here we should probably check if the deployment is already active
        // if it is not active we could just return the old record and make the creation silently succeed
        ...
        if (appDeployment.activatedAt !== null) {
          return {
            type: 'error' as const,
            error: {
              message: 'App deployment already exists and is active',
              details: null,
            },
          };
        }
